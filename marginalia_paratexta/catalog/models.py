...
class OtherCountriesDebut(models.Model):
    creation = models.ForeignKey(MediaCreation, on_delete=models.CASCADE,related_name="mediacreation_other_countriues_debut")
...
class Hipotext(models.Model):
    ...
    innecesary_field =  models.IntegerField("Campo innecesario solo para funcionamiento",default=1, choices=((i,i) for i in range(1, 2)))
...
class Knot(models.Model):
    info = models.CharField('Información del nudo', max_length=10000)
    productos = models.ManyToManyField(Product, blank=True, related_name='related_knots')
    def __str__(self):
        return self.info
    class Meta:
        verbose_name = "Nudo"
        verbose_name_plural = "Nudos"
